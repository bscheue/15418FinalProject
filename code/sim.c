...
coord_t create_start_large(int rb, random_t *seed) {
  float deg = omp_get_thread_num() * (TAU / omp_get_num_threads()) +
              next_random_float(seed, TAU / omp_get_num_threads());
  coord_t res;
  res.i = (int)(rb * sin(deg));
  res.j = (int)(rb * cos(deg));
  return res;
...
#pragma omp parallel for
  ...
  for (i = 0; i < params->w; i++) {
    create_walk(walks[i], params->k, create_start(params->rb, seeds + i),
                seeds + i);
  }
}
...
bool is_sticky(coord_t loc, cluster_t *cluster) {
  ...
  if (locx <= 0 || locx >= cluster->diameter - 1 || locy <= 0 ||
      locy >= cluster->diameter - 1)
    return false;
  return matrix[locx][locy] == STICKY;
}

// return the points where each particle sticks to the cluster (-1 if it doesn't
// stick)
...
bool add_particle(cluster_t *cluster, int i, int j) {
  ...
  if (matrix[locx][locy] == OCCUPIED || locx <= 0 ||
      locx >= cluster->diameter - 1 || locy <= 0 ||
      locy >= cluster->diameter - 1) {
    return false;
  }
  ...
  if (locx >= 1) {
    if (matrix[locx - 1][locy] == EMPTY)
      matrix[locx - 1][locy] = STICKY;
    if (locy >= 1 && matrix[locx - 1][locy - 1] == EMPTY)
      matrix[locx - 1][locy - 1] = STICKY;
    if (locy < cluster->diameter - 1 && matrix[locx - 1][locy + 1] == EMPTY)
      matrix[locx - 1][locy + 1] = STICKY;
  }
  ...
  if (locy >= 1 && matrix[locx][locy - 1] == EMPTY)
    matrix[locx][locy - 1] = STICKY;
  if (locy < cluster->diameter - 1 && matrix[locx][locy + 1] == EMPTY)
    matrix[locx][locy + 1] = STICKY;
  return true;
}
